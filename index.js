...
const mytoken = process.env.MYTOKEN; //Girish_token
let phon_no_id;
let from;
...
const webhook = new WebhookClient({
    channel: {
        url: 'https://idcs-oda-a654ee1daf47498d8225b118e8b62a0b-da4.data.digitalassistant.oci.oraclecloud.com/connectors/v2/listeners/webhook/channels/20082316-10c7-4de7-aaa6-6966ed4d62a3',
        secret: 'KBt8dQeQuhRXj8eFhSzl5lTJY7aZfH2j'
    }
});

webhook
    .on(WebhookEvent.ERROR, err => console.log('Girish webhook Error:', err.message))
    .on(WebhookEvent.MESSAGE_SENT, message => console.log('Girish Message to chatbot:', message));
app.post('/bot/message', webhook.receiver()); // receive bot messages


app.listen(process.env.PORT, () => {
    console.log("Hi Girish. your Webhook is listening");
});
...
app.post("/webhook", (req, res) => { //i want some 
    ...
    if (body_param.object) {
        console.log("Girish i am inside body");
        if (body_param.entry &&
            ...
            console.log("Girish your Message before sending to ODA is ------>" + message);
...
app.get("/", (req, res) => {
    res.status(200).send("Hello Girish this is webhook setup");
});